...
const oldVisibleTasksLogic = `  const uncompleted = tasks.filter(t => !t.done)
...
const newVisibleTasksLogic = `  const isTaskClaimed = (task) => claimedTasks.includes(task.id) || (task.isDynamic && dbUserProgress.includes(task.id));
  const unclaimedTasks = tasks.filter(t => !isTaskClaimed(t));
  const visibleTasks = unclaimedTasks.slice(0, 10);
  
  const completed = tasks.filter(t => t.done).length;
  const progress = tasks.length > 0 ? Math.round((completed / tasks.length) * 100) : 0;`;

dashContent = dashContent.replace(oldVisibleTasksLogic, newVisibleTasksLogic);

fs.writeFileSync(dashPath, dashContent);
console.log('Updated DashboardHome.jsx');

// ----------------------------------------------------
// 2. Update ExploreTasksWidget.jsx
// ----------------------------------------------------
...
const oldWidgetLogic = `        if (!error && data && data.length > 0) {
...
const newWidgetLogic = `        if (!error && data && data.length > 0) {
          const formattedTasks = data.map((t, index) => ({
            id: t.id,
            title: <span dangerouslySetInnerHTML={{ __html: t.title }} />,
            completed: index === 0, // Make the first one completed by default for demo
...
widgetContent = widgetContent.replace(oldWidgetLogic, newWidgetLogic);

const oldToggle = `  const toggleTask = (id) => {
    setTasks(tasks.map(t => t.id === id ? { ...t, completed: !t.completed } : t));
  };`;

const newToggle = `  const toggleTask = (id) => {
...
widgetContent = widgetContent.replace(oldToggle, newToggle);

const oldWidgetFilter = `const completedCount = tasks.filter(t => t.completed).length;`;
const newWidgetFilter = `  const unclaimedTasks = tasks.filter(t => !t.isClaimed);
  const visibleTasks = unclaimedTasks.slice(0, 10);
  const completedCount = tasks.filter(t => t.completed).length;`;

widgetContent = widgetContent.replace(oldWidgetFilter, newWidgetFilter);
...
console.log('Updated ExploreTasksWidget.jsx');

// ----------------------------------------------------
// 3. Update AdminTasks.jsx to add Seed button
// ----------------------------------------------------
const adminPath = 'c:/Softwares/Luter/src/admin/pages/AdminTasks.jsx';
let adminContent = fs.readFileSync(adminPath, 'utf-8');

const seedFunction = `  const handleSeedTasks = async () => {
...
console.log('Updated AdminTasks.jsx');
