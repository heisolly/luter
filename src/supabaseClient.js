...
// Custom storage adapter using cookies for cross-subdomain SSO
export const sharedCookieStorage = {
  ...
  setItem: (key, value) => {
    ...
    // Store for 1 year
    document.cookie = `${key}=${encodeURIComponent(value)}; ${domainString} path=/; max-age=31536000; SameSite=Lax; ${secureString}`;
  },
  removeItem: (key) => {
    if (typeof document === 'undefined') return;
    const domain = getDomain();
