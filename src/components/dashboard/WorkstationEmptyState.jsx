  ...
  const handleAiAction = async (actionType) => {
    ...
    try {
      ...
      const markdownToHtml = (text) => {
        // Remove hallucinated markdown images generated by the LLM
        let preprocessed = text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '');
        
        preprocessed = preprocessed.replace(/\$\$([\s\S]+?)\$\$/g, (match, formula) => {
           return `<div style="text-align: center; margin: 16px 0;"><img src="https://latex.codecogs.com/svg.latex?\\color{black}${encodeURIComponent(formula.trim())}" alt="math" /></div>\n\n`
        })
