...
export default function WorkstationPage() {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchUserStandaloneMaterials(user.id).then(data => {
  ...
  return (
    <CollaborationProvider roomId={roomId} userInfo={profile}>
      <ReadingSpaceProvider>
        ...
        {isShareOpen && (
          <div style={{
            ...
            backgroundColor: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', zIndex: 9999,
            display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}>
            <div style={{
              width: '90%', maxWidth: '420px', backgroundColor: dropdownBg, borderRadius: '16px', padding: '24px',
              boxShadow: '0 24px 48px rgba(0,0,0,0.2)', border: `1px solid ${borderColor}`,
              color: textColor
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 700 }}>Share {noteName}</h3>
                <button onClick={() => setIsShareOpen(false)} style={{
                  background: 'transparent', border: 'none', cursor: 'pointer', color: subTextColor,
              ...
              <div style={{ color: subTextColor, fontSize: '14px', marginBottom: '20px', fontWeight: 500, lineHeight: 1.5 }}>
                Sharing this workspace allows others to collaborate on notes and creations. Your personal chat history is not shared.
              </div>
              
              <div style={{ marginBottom: '20px' }}>
                <input 
                  type="text" 
                  placeholder="Add names or emails to invite" 
                  style={{
                    width: '100%', padding: '12px 16px', borderRadius: '8px', 
                    border: `1px solid ${borderColor}`, backgroundColor: isDark ? '#374151' : '#FFFFFF',
                    color: textColor, outline: 'none', fontSize: '14px', fontWeight: 500, boxSizing: 'border-box'
                  }}
                />
              </div>
              
              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '14px', fontWeight: 700, marginBottom: '12px' }}>Who has access</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', overflow: 'hidden', backgroundColor: isDark ? '#374151' : '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {displayAvatar ? <img src={displayAvatar} style={{width:'100%', height:'100%', objectFit: 'cover'}} alt="You"/> : <UserIcon size={20} color={subTextColor} weight="fill"/>}
                    </div>
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: 600 }}>{displayName} (you)</div>
                      <div style={{ fontSize: '12px', color: subTextColor, fontWeight: 500 }}>Owner</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <button 
                ...
                backgroundColor: copiedToast ? '#10B981' : '#0066FF', color: 'white', fontWeight: 700, cursor: 'pointer',
                fontSize: '15px', transition: 'background-color 0.2s', marginBottom: '24px'
              }}
              onMouseEnter={e => { if(!copiedToast) e.currentTarget.style.backgroundColor = '#005CE6' }}
              onMouseLeave={e => { if(!copiedToast) e.currentTarget.style.backgroundColor = '#0066FF' }}
              >
                {copiedToast ? 'Copied to clipboard!' : 'Copy link'}
              </button>
