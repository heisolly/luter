...
const TabsSubtleContext = createContext(null);
...
const TabsSubtle = forwardRef(
  ({ children, selectedIndex, onSelect, idPrefix: idPrefixProp, activeLabel = false, className, ...props }, ref) => {
    ...
    const registerTab = useCallback((index, element) => {
    ...
    return (
      <TabsSubtleContext.Provider
        ...
        <div
          ...
          onKeyDown={(e) => {
            const items = Array.from(containerRef.current?.querySelectorAll('[role="tab"]') ?? []);
...
const TabsSubtleItem = forwardRef(({ icon: Icon, label, index, className, ...props }, ref) => {
