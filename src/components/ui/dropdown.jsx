...
const Dropdown = forwardRef(({ children, checkedIndex, className, ...props }, ref) => {
...
const DropdownTrigger = forwardRef(({ render, children, disabled, ...props }, ref) => {
...
const DropdownContent = forwardRef((
  ...
  return (
    <DropdownMenuPrimitive.Portal forceMount>
      <DropdownMenuPrimitive.Content asChild forceMount side={side} align={align} sideOffset={sideOffset}>
        <motion.div
            ...
            <Elevated
              ...
              onFocus={(e) => {
                ...
                if (indexAttr != null) {
                  const idx = Number(indexAttr);
                  setActiveIndex(idx);
                  setFocusedIndex((e.target).matches(":focus-visible")
                    ? idx
                    : null);
                }
              }}
              ...
              className={cn(// min-w tracks the trigger; the available-height guard maps
              // Base UI's --available-height to Radix's equivalent var.
              `relative flex flex-col gap-0.5 w-72 max-w-full min-w-[var(--radix-dropdown-menu-trigger-width)] max-h-[min(480px,var(--radix-dropdown-menu-content-available-height))] overflow-y-auto ${shape.container} p-1 select-none outline-none`, className)}>
              {/* Selected background */}
              <AnimatePresence>
                {checkedRect && (
                  <motion.div
                    className={`absolute ${shape.bg} bg-gray-100 dark:bg-gray-800 pointer-events-none`}
                    initial={false}
                    animate={{
                      top: checkedRect.top,
                      left: checkedRect.left,
              ...
              <AnimatePresence>
                {activeRect && (
                  <motion.div
                    key={sessionRef.current}
                    className={`absolute ${shape.bg} bg-gray-100 dark:bg-[#1e2d45] pointer-events-none`}
                    initial={{
                      opacity: 0,
                      top: checkedRect?.top ?? activeRect.top,
                      left: checkedRect?.left ?? activeRect.left,
